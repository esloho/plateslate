defmodule PlateSlateWeb.Schema.Query.MenuItemTest do
  ...
  """
  @variables %{filter: %{"priceAbove" => "5"}}
  test "menuItems filtered by decimal price" do
    response = get(build_conn(), "/api", query: @query, variables: @variables)

    assert %{"data" => %{"menuItems" => results}} = json_response(response, 200)
    assert length(results) == 2
    assert Enum.find(results, &(&1["price"] == "5.5"))
  end
end
